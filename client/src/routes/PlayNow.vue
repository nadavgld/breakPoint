...
<div>
  <div class="play-container">
    <div v-for="(user, index) in participants"
        :key="user.username" class="user" >
      <i v-if="index > 0" class="users fas fa-user"></i>
      <i v-if="index ==0" class="users fas fa-user-secret"></i>
      {{user.username}}
  </div>
  </div>
  <v-btn v-if="showStartGame" class="mt start-game-button" color="blue" @click="startGame">Start Game</v-btn>
</div>
</template>
...
export default {
  data () {
    return {
      name: 'playNow',
      participants: [{username: "gal", userId:"i"}, {username: "Inbar", userId: "ii"}]
    }
  },
   computed: {
